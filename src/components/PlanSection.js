      ...
      <div className="w-full mb-10">
        <div className="image-container relative overflow-hidden max-h-72">
          ...
          <div className="overlay absolute bottom-0 left-0 right-0 bg-gray-800 bg-opacity-55 p-5">
            <h1 className="text-white text-4xl font-serif mb-5">Plan Your Meal</h1> {/* Added padding below title */}
          </div>
        </div>
      </div>
    ...
    {["appetizers", "starters", "mainDish", "dessert"].map((category) => (
      <div key={category} className="mb-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {recipesData.filter((recipe) => recipe.category === category).map((recipe) => (
            <RecipeCard
              key={recipe.id}
              recipe={recipe}
